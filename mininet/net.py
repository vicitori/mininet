...
class Mininet(object):
    ...
    def addHost(self, name, cls=None, **params):
        ...
        defaults = {
            "ip": ipAdd(self.nextIP, ipBaseNum=self.ipBaseNum, prefixLen=self.prefixLen)
            + "/%s" % self.prefixLen
        }
        if self.autoSetMacs:
        ...
        if nodes is None:
            nodes = (
                ...
                if node in self.hosts
                    ...
                    else (self.controllers if node in self.controllers else [])
                )
            )
    ...
    def addNAT(self, name="nat0", connect=True, inNamespace=False, **params):
        ...
        nat = self.addHost(
            name, cls=NAT, inNamespace=inNamespace, subnet=self.ipBase, **params
        )
    ...
    def randMac():
        ...
        return macColonHex(
            random.randint(1, 2**48 - 1) & 0xFEFFFFFFFFFF | 0x020000000000
        )
    ...
    def linksBetween(self, node1, node2):
        ...
        return [
            ...
            if (node1, node2)
            in ((link.intf1.node, link.intf2.node), (link.intf2.node, link.intf1.node))
        ]
    ...
    def configureControlNetwork(self):
        ...
        raise Exception(
            "configureControlNetwork: " "should be overriden in subclass", self
        )
    ...
    def iperf(
        self, hosts=None, l4Type="TCP", udpBw="10M", fmt=None, seconds=5, port=5001
    ):
        ...
        cliout = client.cmd(
            iperfArgs + "-t %d -c " % seconds + server.IP() + " " + bwArgs
        )
