...
def errFail(*cmd, **kwargs):
    ...
    if ret:
        raise Exception("errFail: %s failed with return code %s: %s" % (cmd, ret, err))
...
def makeIntfPair(
    ...
    if addr1 is None and addr2 is None:
        ...
        cmdOutput = runCmd(
            ...
            "type veth peer name %s "
            "address %s "
            "netns %s" % (intf1, addr1, intf2, addr2, netns)
        )
    if cmdOutput:
        raise Exception(
...
def pmonitor(popens, timeoutms=500, readline=True, readmax=1024):
                ...
                if event & (POLLIN | POLLHUP):
                    while True:
                        try:
                            f = popen.stdout
                            line = decoder.decode(
                                f.readline() if readline else f.read(readmax)
                            )
                        except IOError:
                            line = ""
                        if line == "":
...
def mountCgroups(cgcontrol="cpu cpuacct cpuset"):
    ...
    if result.ret or result.err or any(c not in result.out for c in cglist):
...
def customClass(classes, argStr):
    ...
    if not cls:
        raise Exception(
            "error: %s is unknown - please specify one of %s" % (cname, classes.keys())
        )
...
def specialClass(cls, prepend=None, append=None, defaults=None, override=None):
    ...
    class CustomClass(cls):
        ...
        def __init__(self, *args, **params):
            ...
            cls.__init__(
                self, *(list(prepend) + list(args) + list(append)), **newparams
            )
