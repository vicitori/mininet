    ...
    def popen(self, *args, **kwargs):
        ...
        if self.sched == "rt":
            if int(self.cgroupGet("rt_runtime_us", "cpu")) <= 0:
                ...
                debug(
                    "*** error: not enough cpu time available for %s." % self.name,
...
class UserSwitch(Switch):
    ...
    def connected(self):
        ...
        return (
            "remote.is-connected=true" in status and "local.is-connected=true" in status
        )
...
class OVSSwitch(Switch):
    ...
    def controllerUUIDs(self, update=False):
        ...
        if not self._uuids or update:
            controllers = self.cmd(
                "ovs-vsctl -- get Bridge", self, "Controller"
            ).strip()
...
class IVSSwitch(Switch):
    ...
    def setup(cls):
        ...
        pathCheck(
            "ivs-ctl", "ivs", moduleName="Indigo Virtual Switch (projectfloodlight.org)"
        )
    ...
    def dpctl(self, *args):
        ...
        return self.cmd(
            "ovs-ofctl " + " ".join(args) + " tcp:127.0.0.1:%i" % self.listenPort
        )
...
class Controller(Node):
    ...
    def checkListening(self):
        ...
        if "telnet" not in out or returnCode != 0:
            raise Exception(
                "Error running telnet to check for listening "
        ...
        if "Connected" in listening:
            ...
            raise Exception(
                "Please shut down the controller which is"
                " running on port %d:\n" % self.port + "\n".join(clist)
            )
...
class OVSController(Controller):
    ...
    def isAvailable(cls):
        return (
            ...
            or which("test-controller")
            or which("ovs-testcontroller")
        )
...
class NOX(Controller):
    ...
    def __init__(self, name, *noxArgs, **kwargs):
        ...
        Controller.__init__(
            ...
            cargs="--libdir=/usr/local/lib -v " "-i ptcp:%s " + " ".join(noxArgs),
            cdir=noxCoreDir,
            **kwargs
        )
...
class Ryu(Controller):
    ...
    def __init__(
        self, name, ryuArgs="ryu.app.simple_switch", command="ryu run", **kwargs
    ):
        """Init.
        name: name to give controller.
