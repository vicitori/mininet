...
class testOptionsTopoCommon(object):
    ...
    def runOptionsTopoTest(self, n, msg, hopts=None, lopts=None):
        ...
        mn = Mininet(
            topo=SingleSwitchOptionsTopo(n=n, hopts=hopts, lopts=lopts),
            host=CPULimitedHost,
            link=TCLink,
        ...
        msg += (
            "%s%% of pings were dropped during mininet.ping().\n"
    ...
    def testCPULimits(self):
        ...
        mn = Mininet(
            SingleSwitchOptionsTopo(n=N, hopts=hopts),
            host=CPULimitedHost,
            switch=self.switchClass,
            waitConnected=True,
        )
        ...
        mn.stop()
        hostUsage = "\n".join(
            "h%s: %s" % (n + 1, results[(n - 1) * 5 : (n * 5) - 1]) for n in range(N)
        )
        hoptsStr = ", ".join("%s: %s" % (opt, value) for opt, value in hopts.items())
        msg = (
    ...
    def testLinkBandwidth(self):
        ...
        mn = Mininet(
            SingleSwitchOptionsTopo(n=N, lopts=lopts),
            link=TCLink,
            switch=self.switchClass,
            waitConnected=True,
        )
    ...
    def testLinkDuplicate(self):
        DUPLICATE_PERCENT = 99
        COUNT = 20

        lopts = {"duplicate": DUPLICATE_PERCENT, "use_htb": True}
        ...
        replies = re.findall(r"bytes from", output)

        received = len(replies)

        msg = (
            f"\nTesting packet duplication with {DUPLICATE_PERCENT}% rate\n"
            f"Sent: {COUNT}\n"
            f"Received lines (including duplicates): {received}\n"
            f"Expected: received > sent (i.e. duplicates exist)\n"
            f"Output:\n{output}\n"
        )

        self.assertGreater(received, COUNT, msg)
