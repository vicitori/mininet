...
class testWalkthrough(unittest.TestCase):
    ...
    def testHostCommands(self):
        ...
        self.assertTrue(
            len(diffs) <= 2, 'h1 and s1 "ps" output differ too much: %s' % diffs
        )
        p.sendline("exit")
        p.wait()
    ...
    def testSimpleHTTP(self):
        ...
        p.sendline(
            "px from mininet.util import waitListening;"
    ...
    def testLinkChange(self):
        ...
        p.expect(
            r"rtt min/avg/max/mdev = " r"([\d\.]+)/([\d\.]+)/([\d\.]+)/([\d\.]+) ms"
        )
    ...
    @unittest.skipUnless(
        os.path.exists("/tmp/pox") or "1 received" in quietRun("ping -c 1 github.com"),
