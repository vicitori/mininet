...
class NAT(Node):
    ...
    def config(self, **params):
        ...
        # Install NAT rules
        self.cmd(
            "iptables -I FORWARD", "-i", self.localIntf, "-d", self.subnet, "-j DROP"
        )
        self.cmd(
            "iptables -A FORWARD", "-i", self.localIntf, "-s", self.subnet, "-j ACCEPT"
        )
        self.cmd(
            "iptables -A FORWARD", "-o", self.localIntf, "-d", self.subnet, "-j ACCEPT"
        )
        self.cmd(
    ...
    def terminate(self):
        ...
        # Remote NAT rules
        self.cmd(
            "iptables -D FORWARD", "-i", self.localIntf, "-d", self.subnet, "-j DROP"
        )
        self.cmd(
            "iptables -D FORWARD", "-i", self.localIntf, "-s", self.subnet, "-j ACCEPT"
        )
        self.cmd(
            "iptables -D FORWARD", "-o", self.localIntf, "-d", self.subnet, "-j ACCEPT"
        )
        self.cmd(
