...
class TCIntf(Intf):
    ...
    as well as delay, loss, duplicate and max queue length"""
    ...
    @staticmethod
    def delayCmds(
        parent, delay=None, jitter=None, loss=None, duplicate=None, max_queue_size=None
    ):
        "Internal method: return tc commands for delay, loss and duplicate"
        cmds = []
        if loss and (loss < 0 or loss > 100):
            error("Bad loss percentage", loss, "%%\n")
        if duplicate and (duplicate < 0 or duplicate > 100):
            error("Bad duplicate percentage", duplicate, "%%\n")
        else:
            # Delay/jitter/loss/duplicate/max queue size
            netemargs = "%s%s%s%s%s" % (
                "delay %s " % delay if delay is not None else "",
                "%s " % jitter if jitter is not None else "",
                "loss %.5f " % loss if (loss is not None and loss > 0) else "",
                (
                    "duplicate %.5f " % duplicate
    ...
    def config(  # pylint: disable=arguments-renamed,arguments-differ
        ...
        **params
        ...
        # Delay/jitter/loss/duplicate/max_queue_size using netem
        ...
        stuff = (
            ...
            + (["%.5f%% duplicate" % duplicate] if duplicate is not None else [])
