...
def findUser():
    ...
    return (
        ...
        os.environ.get("SUDO_USER", False)
        or
        # Logged-in user (if we have a tty)
        (quietRun("who am i").split() or [False])[0]
        or
...
class RemoteLink(Link):
    ...
    def makeIntfPair(
        self,  # pylint: disable=arguments-renamed
        intfname1,
        intfname2,
        addr1=None,
        addr2=None,
...
class RemoteGRELink(RemoteLink):
    ...
    def makeTunnel(self, node1, node2, intfname1, intfname2, addr1=None, addr2=None):
        ...
        for node, intfname, addr, tun in [
            (node1, intfname1, addr1, tun1),
...
class MininetCluster(Mininet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.serverIP:
            self.serverIP = {
                server: RemoteMixin.findServerIP(server) for server in self.servers
            }
    ...
    def addController(self, *args, **kwargs):
        ...
        if not isinstance(controller, Controller) or not self.isLoopback(
            controller.IP()
        ):
            return controller
        ...
        for remoteIP in serverIPs:
            ...
            route = controller.cmd(
                "ip route get", remoteIP, r'| egrep -o "dev\s[^[:space:]]+"'
            )
            if not route:
                raise Exception(
                    "addController: no route from", controller, "to", remoteIP
                )
        ...
        if intf == "lo":
            raise Exception(
                "addController: could not find external "
...
def ClusterController(*args, **kwargs):
    ...
    if not intf:
        ...
        if intf == "lo":
            raise Exception(
                "Could not find non-loopback interface" "for %s" % controller
            )
    Intf(intf, node=controller).updateIP()
    return controller
...
def testRemoteSwitches(remote=remoteServer, link=RemoteGRELink):
    ...
    net = MininetCluster(
        topo=topo, servers=servers, link=link, placement=RoundRobinPlacer
    )
    net.start()
    net.pingAll()
    net.stop()
...
def testMininetCluster(remote=remoteServer, link=RemoteGRELink):
    ...
    net = MininetCluster(
        topo=topo, servers=servers, link=link, placement=SwitchBinPlacer
    )
    net.start()
    net.pingAll()
    net.stop()
