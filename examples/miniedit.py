...
class PrefsDialog(tkSimpleDialog.Dialog):
    ...
    def body(self, master):
        ...
        self.switchTypeMenu = OptionMenu(
            self.leftfieldFrame,
            self.switchType,
            "Open vSwitch Kernel Mode",
            "Indigo Virtual Switch",
            "Userspace Switch",
            "Userspace Switch inNamespace",
        )
        ...
        self.nFrame = LabelFrame(
            self.rightfieldFrame,
            text="NetFlow Profile for Open vSwitch",
            padx=5,
            pady=5,
        )
...
class HostDialog(CustomDialog):
    ...
    def apply(self):
        ...
        for row in range(self.vlanTableFrame.rows):
            if (
                len(self.vlanTableFrame.get(row, 0)) > 0
                and len(self.vlanTableFrame.get(row, 1)) > 0
                and row > 0
            ):
                vlanInterfaces.append(
                    [self.vlanTableFrame.get(row, 0), self.vlanTableFrame.get(row, 1)]
                )
        privateDirectories = []
        for row in range(self.mountTableFrame.rows):
            if len(self.mountTableFrame.get(row, 0)) > 0 and row > 0:
                if len(self.mountTableFrame.get(row, 1)) > 0:
                    privateDirectories.append(
                        (
                            self.mountTableFrame.get(row, 0),
...
class MiniEdit(Frame):
    ...
    def createMenubar(self):
        ...
        fileMenu.add_command(
            label="Export Level 2 Script", font=font, command=self.exportScript
        )
        ...
        editMenu.add_command(
            label="Cut", font=font, command=lambda: self.deleteSelection(None)
        )
    ...
    def convertJsonUnicode(self, text):
        ...
        if isinstance(text, dict):
            return {
                self.convertJsonUnicode(key): self.convertJsonUnicode(value)
    ...
    def loadTopology(self):
        ...
        for link in links:
            ...
            self.link = self.canvas.create_line(
                sx, sy, dx, dy, width=4, fill="blue", tag="link"
            )
    ...
    def exportScript(self):
        ...
        if len(fileName) > 0:
            ...
            if len(self.appPrefs["dpctl"]) > 0:
                f.write(
                    "                   listenPort=" + self.appPrefs["dpctl"] + ",\n"
                )
            f.write("                   build=False,\n")
            ...
            if len(nflowValues["nflowTarget"]) > 0:
                ...
                if nflowEnabled:
                    nflowCmd = (
                        "ovs-vsctl -- --id=@MiniEditNF create NetFlow "
                        + 'target=\\"'
                        + nflowValues["nflowTarget"]
                        + '\\" '
                        + "active-timeout="
                        + nflowValues["nflowTimeout"]
                    )
            ...
            if len(sflowValues["sflowTarget"]) > 0:
                ...
                if sflowEnabled:
                    sflowCmd = (
                        "ovs-vsctl -- --id=@MiniEditSF create sFlow "
                        + 'target=\\"'
                        + sflowValues["sflowTarget"]
                        + '\\" '
                        + "header="
                        + sflowValues["sflowHeader"]
                        + " "
                        + "sampling="
                        + sflowValues["sflowSampling"]
                        + " "
                        + "polling="
                        + sflowValues["sflowPolling"]
                    )
                    f.write("    \n")
                    f.write(
            ...
            for widget, item in self.widgetToItem:
                ...
                if "Host" in tags:
                    ...
                    if "stopCommand" in opts:
                        f.write(
                            "    " + name + ".cmdPrint('" + opts["stopCommand"] + "')\n"
                        )
                if "Switch" in tags:
                    opts = self.switchOpts[name]
                    # Run User Defined Stop Command
    ...
    def startLink(self, event):
        ...
        self.link = self.canvas.create_line(
            x, y, x, y, width=4, fill="blue", tag="link"
        )
    ...
    def finishLink(self, event):
        ...
        if (
            ...
            or dest is None
            or source == dest
            or dest in source.links
            or source in dest.links
        ):
        ...
        if (
            ("Host" in stags and "Host" in dtags)
            or ("Controller" in dtags and "LegacyRouter" in stags)
            or ("Controller" in stags and "LegacyRouter" in dtags)
            or ("Controller" in dtags and "LegacySwitch" in stags)
            or ("Controller" in stags and "LegacySwitch" in dtags)
            or ("Controller" in dtags and "Host" in stags)
            or ("Controller" in stags and "Host" in dtags)
            or ("Controller" in stags and "Controller" in dtags)
        ):
            self.releaseNetLink(event)
            return
    ...
    def about(self):
        ...
        if about is None:
            ...
            line5 = Entry(
                about, font="Helvetica 9", bg=bg, width=len(www), justify=CENTER
            )
            line5.insert(0, www)
    ...
    def switchDetails(self, _ignore=None):
        ...
        if switchBox.result:
            ...
            if len(switchBox.result["externalInterfaces"]) > 0:
                newSwitchOpts["externalInterfaces"] = switchBox.result[
                    "externalInterfaces"
                ]
    ...
    def listBridge(self, _ignore=None):
        ...
        if "Switch" in tags or "LegacySwitch" in tags:
            call(
                [
                    "xterm -T 'Bridge Details' -sb -sl 2000 -e 'ovs-vsctl list bridge "
                    + name
    ...
    def ovsShow(_ignore=None):
        call(
            [
                "xterm -T 'OVS Summary' -sb -sl 2000 -e 'ovs-vsctl show; read -p \"Press Enter to close\"' &"
            ],
            shell=True,
        )
            ...
            if "Switch" in tags:
                ...
                if "cores" in opts or "cpu" in opts:
                    if "privateDirectory" in opts:
                        hostCls = partial(
                            CPULimitedHost, privateDirs=opts["privateDirectory"]
                        )
                    else:
                ...
                debug(hostCls, "\n")
                newHost = net.addHost(
                    name, cls=hostCls, ip=ip, defaultRoute=defaultRoute
                )
    ...
    def build(self):
        ...
        net = Mininet(
            topo=None, listenPort=dpctl, build=False, ipBase=self.appPrefs["ipBase"]
        )
    ...
    def parseArgs(self):
        ...
        desc = (
            "The %prog utility creates Mininet network from the\n"
    ...
    def importTopo(self):
        ...
        for controller in importNet.controllers:
            ...
            self.addNode(
                "Controller", self.controllerCount, float(x), float(currentY), name=name
            )
        ...
        for switch in importNet.switches:
            ...
            self.addNode(
                "Switch", self.switchCount, float(x), float(currentY), name=name
            )
            icon = self.findWidgetByName(name)
            icon.bind("<Button-3>", self.do_switchPopup)
