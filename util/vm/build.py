...
isoURLs = {
    "precise64server": "http://mirrors.kernel.org/ubuntu-releases/12.04/"
    "ubuntu-12.04.5-server-amd64.iso",
    "trusty32server": "http://mirrors.kernel.org/ubuntu-releases/14.04/"
    "ubuntu-14.04.4-server-i386.iso",
    "trusty64server": "http://mirrors.kernel.org/ubuntu-releases/14.04/"
    "ubuntu-14.04.4-server-amd64.iso",
    "xenial32server": "http://mirrors.kernel.org/ubuntu-releases/16.04/"
    "ubuntu-16.04.6-server-i386.iso",
    "xenial64server": "http://mirrors.kernel.org/ubuntu-releases/16.04/"
...
def findBaseImage(flavor, size="8G"):
    ...
    if path.exists(image):
        ...
        if perms != 0o444:
            raise Exception(
                "Error - base image %s is writable." % image
...
def interact(vm, tests, pre="", post="", prompt=Prompt, clean=True):
    ...
    vm.sendline(
        "sudo sed -i -e 's/^GRUB_TERMINAL=serial/#GRUB_TERMINAL=serial/' "
...
def generateOVF(
    ...
    cpus=1,
    vmname="Mininet-VM",
    vminfo="A Mininet Virtual Machine",
):
...
def build(flavor="raring32server", tests=None, pre="", post="", memory=1024):
    ...
    ovf = generateOVF(
        name=ovfname, osname=osname, osid=osid, diskname=vmdk, disksize=size
    )
...
def testDict():
    ...
    fdict = dict(
        [
            (fname[:-trim], f)
            for fname, f in inspect.getmembers(modules[__name__], inspect.isfunction)
...
def parseArgs():
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="list valid build flavors and tests"
    )
    parser.add_argument(
        "-c",
        "--clean",
    ...
    parser.add_argument(
        "flavor", nargs="*", help="VM flavor(s) to build; " + buildFlavorString()
    )
    parser.add_argument(
        "-z", "--zip", action="store_true", help="archive .ovf and .vmdk into .zip file"
    )
    ...
    parser.add_argument(
        "-u", "--chown", metavar="user", help="specify an owner for build directory"
    )
